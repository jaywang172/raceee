...
print("--- Final Model with Top 20 Features ---")
print("Loading data...")
try:
...
print("\n--- Part 3: Selecting Top 20 Features & Training Model ---")

# Top 20 features from the last analysis run
top_20_features = [
    'first_txn_date', 'days_since_first_txn', 'unique_to_acct_count', 'from_mean_amt',
    'from_max_amt', 'from_max_vs_mean_ratio', 'channel_UNK_ratio', 'channel_03_ratio',
    'to_amt_sum_30d', 'unique_from_acct_count', 'to_amt_mean_30d', 'round_number_txn_ratio',
    'to_count_30d', 'to_amt_sum_14d', 'night_txn_ratio', 'from_amt_sum_30d',
    'to_amt_mean_14d', 'from_count_30d', 'to_amt_mean_7d', 'diff_from_mean_amt_3d_vs_30d'
]

print(f"Selected {len(top_20_features)} features for training.")
...
lgb_params = {
    ...
    'learning_rate': 0.02, 'num_leaves': 31, # Reduced leaves for simpler model
    'max_depth': 7, # Added max_depth to control complexity
...
# Use only top 20 features for prediction
test_features_df = features_df.reindex(np.union1d(features_df.index, test_accts)).loc[test_accts][top_20_features].copy()
...
submission_df.to_csv('submission_top20.csv', index=False)

print("\n--- Submission File 'submission_top20.csv' Generated ---")
